...
命令面板执行 **Ask AI：提问并插入到笔记**，输入问题（如“帮我为这篇笔记写一个开头”），生成结果会实时显示，点击“插入到笔记”写入光标处，也可以复制或停止生成。**无需手动选中文本**：提问时会自动附带上下文发送给模型，优先级如下：

1. **有选中文本**：优先把选中内容作为核心上下文发送（左键选中后提问，模型一定能看到你选中的部分）
2. 未选中且笔记非空：附带整篇笔记内容
3. 笔记为空：明确告知模型“笔记为空”，不会静默丢失上下文

弹窗中会显示一行说明（如“将附带笔记《xxx》内容（123 字）”），发送前即可确认模型实际收到的范围。可在设置中关闭自动附带，或调整“上下文最大字符数”（默认 8000 字符，设置异常时自动回退默认值，避免模型“看不到”笔记）。
